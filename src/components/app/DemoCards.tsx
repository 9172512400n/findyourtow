...
export function ActionCard({ title, copy, href, action, meta }: { title: string; copy: string; href?: string; action?: string; meta?: string }) {
  const content = <div className="group flex min-h-24 items-center justify-between gap-4 rounded-[1.6rem] border border-white/10 bg-white/[0.055] p-5 text-left transition hover:border-white/25 hover:bg-white/[0.08]"><div><p className="font-black text-white">{title}</p><p className="mt-1 text-sm font-bold leading-5 text-white/48">{copy}</p>{meta && <p className="mt-2 text-xs font-black uppercase tracking-[0.18em] text-white/42">{meta}</p>}</div><span className="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-white text-black transition group-hover:scale-105"><ArrowRight size={18} /></span></div>;
