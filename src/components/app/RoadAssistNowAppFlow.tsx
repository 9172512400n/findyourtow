...
function ConfirmStep({ data, provider, quote, submitting, error, onChange, onNext }: { data: ReturnType<typeof useRequestFlowStore.getState>["data"]; provider: AvailableDriver; quote: ReturnType<typeof calculateQuote>; submitting: boolean; error: string | null; onChange: (next: Partial<ReturnType<typeof useRequestFlowStore.getState>["data"]>) => void; onNext: () => void }) {
  ...
  return <div className="space-y-4"><StepTitle title="Confirm reservation" copy="Account and payment are required before dispatch. Confirm the account contact info your provider will use." /><div className="grid gap-3 rounded-[1.45rem] border border-blue-300/16 bg-blue-400/10 p-4"><label className="text-xs font-black uppercase tracking-[0.18em] text-blue-100/54">Account name<input value={data.customerName} onChange={(event) => onChange({ customerName: event.target.value })} placeholder="Account name" className="mt-2 w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-base font-black text-white outline-none focus:border-blue-300" /></label><label className="text-xs font-black uppercase tracking-[0.18em] text-blue-100/54">Account phone<input value={data.phone} onChange={(event) => onChange({ phone: event.target.value })} placeholder="Account mobile phone" inputMode="tel" className="mt-2 w-full rounded-2xl border border-white/10 bg-black/35 px-4 py-3 text-base font-black text-white outline-none focus:border-blue-300" /></label></div><div className="rounded-[1.6rem] bg-white/[0.065] p-5 text-sm font-bold text-white/70"><p className="text-xl font-black text-white">{service}</p><p className="mt-3">Pickup: {data.pickupAddress || "Current location"}</p>{data.dropoffAddress && <p className="mt-2">Destination: {data.dropoffAddress}</p>}<p className="mt-2">Vehicle: {[data.vehicleYear, data.vehicleColor, data.vehicleMake, data.vehicleModel].filter(Boolean).join(" ") || data.vehicleType}</p><p className="mt-2">Provider search: {provider.name} · {provider.truckType} is the closest current match</p><p className="mt-2">Payment status: Authorized · {formatMoney(quote.totalCents)}</p><p className="mt-2">Estimated arrival: {provider.etaMinutes} minutes</p></div>{error && <div className="rounded-[1.25rem] border border-red-300/20 bg-red-500/10 px-4 py-3 text-sm font-black text-red-100">{error}</div>}<Button className="w-full" onClick={onNext} disabled={submitting}>{submitting ? "Saving request…" : "Confirm reservation"}</Button></div>;
}
